/**
 * @fileoverview Script de prueba standalone para ejecutar el flujo completo de procesamiento de un shipment
 * 
 * Este script NO requiere Firebase. Usa un access token de MELI directamente.
 * 
 * Uso:
 *   1. Obtén un access token de MELI (desde Firebase Console > Firestore > oAuthResponsesMercadoLibre)
 *   2. Configura la variable MELI_ACCESS_TOKEN abajo
 *   3. npx ts-node scripts/test-shipment-flow-standalone.ts
 */
...
// ====================================================================
const MELI_ACCESS_TOKEN = "TU_ACCESS_TOKEN_AQUI"; // ⚠️ Reemplazar con el access token real
const SHIPMENT_ID_TO_TEST = "46915311421"; // Cambiar según necesidad
const TRACE_ID = `TEST-${Date.now()}`;
const MELI_API_URL = "https://api.mercadolibre.com";

// ====================================================================
// FUNCIONES AUXILIARES PARA LLAMADAS A MELI
// ====================================================================

async function fetchShipmentDetailsStandalone(shipmentId: number, traceId: string) {
  console.log(`[Trace: ${traceId}] Consultando shipment ID: ${shipmentId}`);
  
  const url = `${MELI_API_URL}/shipments/${shipmentId}`;
...
async function fetchPackDetailsStandalone(packId: string, traceId: string) {
  console.log(`[Trace: ${traceId}] Consultando pack ID: ${packId}`);
  
  const url = `${MELI_API_URL}/packs/${packId}`;
  const response = await axios.get(url, {
    headers: { Authorization: `Bearer ${MELI_ACCESS_TOKEN}` },
  });
...
async function fetchOrderDetailsStandalone(orderPath: string, traceId: string) {
  console.log(`[Trace: ${traceId}] Consultando order: ${orderPath}`);
  
  const url = `${MELI_API_URL}${orderPath}`;
  const response = await axios.get(url, {
    headers: { Authorization: `Bearer ${MELI_ACCESS_TOKEN}` },
  });
...
async function testShipmentFlow() {
  try {
    ...
    if (MELI_ACCESS_TOKEN === "TU_ACCESS_TOKEN_AQUI") {
      console.error("\n❌ ERROR: Debes configurar MELI_ACCESS_TOKEN en el script");
      console.log("\n📝 Para obtener el access token:");
      console.log("   1. Ve a Firebase Console");
      console.log("   2. Firestore Database > oAuthResponsesMercadoLibre");
      console.log("   3. Copia el campo 'access_token'");
      console.log("   4. Reemplaza 'TU_ACCESS_TOKEN_AQUI' en este script\n");
      process.exit(1);
    }
    ...
    const shipment = await fetchShipmentDetailsStandalone(Number(SHIPMENT_ID_TO_TEST), TRACE_ID);
    ...
    try {
      const packDetails = await fetchPackDetailsStandalone(packId, TRACE_ID);
      ...
      log("📦", "Detalles del pack:", {
        ...
        orders: packDetails.orders?.map((o: any) => ({
          id: o.id,
      ...
      if (packDetails.orders && packDetails.orders.length > 0) {
        for (const order of packDetails.orders) {
          ...
          try {
            const orderDetails = await fetchOrderDetailsStandalone(`/orders/${order.id}`, TRACE_ID);
      ...
      if (error.response?.status === 400) {
        ...
        if (isIndividualOrder) {
          ...
          try {
            const orderDetails = await fetchOrderDetailsStandalone(`/orders/${packId}`, TRACE_ID);
    ...
    console.log(`  - Tipo logístico: ${shipment.logistic_type}`);
